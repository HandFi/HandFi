...
def on_close(event):
	...
	if (SAVE):
		...
		np.savetxt("./dataset/label" + label_input.upper() + str(time_stamp) + ".csv", points_list, delimiter=',', header=headers, comments='')
...
def get_points():
	...
	# img = frame.images[1]
	# # wrap image data in numpy array
	# i_address = int(img.data_pointer)
	# ctype_array_def = ctypes.c_ubyte * img.height * img.width
	# # as ctypes array
	# as_ctype_array = ctype_array_def.from_address(i_address)
	# # as numpy array
	# as_numpy_array = np.ctypeslib.as_array(as_ctype_array)
	# img = np.reshape(as_numpy_array, (img.height, img.width))
	...
	if image.is_valid:
		...
		# display images
		# cv.imshow('right Camera', im_bw)
		# sio.savemat('test.mat', {'mydata': im_bw})
		# pdb.set_trace()
		
		# cv.imwrite('test.jpg', im_bw)
	...
	# print(hand.palm_position.x, hand.palm_position.y, hand.palm_position.z)
