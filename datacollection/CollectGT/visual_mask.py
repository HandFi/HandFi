...
def on_close(event):
	...
	if (SAVE):
		...
		np.savetxt("./dataset200/" + label_input.upper() + "/label" + label_input.upper() + str(now.strftime("%Y-%m-%d-%H-%M-%S")) + ".csv", points_list, delimiter=',', header=headers, comments='')
...
def get_points():
	...
	if image.is_valid:
		...
		now = datetime.now()	
		cv.imwrite("./dataset/" + label_input.upper() + str(now.strftime("%Y-%m-%d-%H-%M-%S-%f")) + ".jpg", undistorted_right)
		print(True)
...
def main():	
	time.sleep(0.5)
	for i in range(10): # one sec 10 times
		# start=time.time()
		points = get_points()
		print(i)
		if (SAVE):
